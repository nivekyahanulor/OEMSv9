...
$query .= "SELECT * FROM $RESERVATION_TABLE WHERE status = 'Pending' AND ";
		...
		$sub_array[] = '
			<div class="btn-group btn-group-md elevation-2">
				<a href="#" class="btn btn-dark status text-bold" name="status" id="'.$row["id"].'" 
					...
					data-reservation_id="'.$row["reservation_id"].'" data-status="Reserve" 
					data-toggle="tooltip" data-placement="top" title="Reserve">
						Reserve
				</a>

				<a href="#" class="btn btn-default status text-bold" name="status" id="'.$row["id"].'" 
					data-customer="'.$clients["last_name"].", ".$clients["first_name"]." ".$clients["middle_name"].'" 
					data-reservation_id="'.$row["reservation_id"].'" data-status="Decline" 
...
{
	$statement = $connect->prepare("SELECT * FROM $TABLE WHERE status = 'Pending' ");
	$statement->execute();
