...
$query .= "SELECT * FROM $RESERVATION_TABLE WHERE status = 'Cancelled' AND ";
...
foreach($result as $row)
{
	...
	$sub_array[] = $row['reservation_id'].$reason;
...
{
	$statement = $connect->prepare("SELECT * FROM $TABLE WHERE status = 'Cancelled' ");
	$statement->execute();
